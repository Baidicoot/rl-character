...
from code_data.generation.executor import test_solution, compare_outputs, values_equal, safe_eval_actual
...
"""
    ...
    async def test_integer_comparisons(self, sample_solution_code):
        ...
        for func_call, expected, should_pass, desc in test_cases:
            ...
            passed, error = await test_solution(sample_solution_code, func_name, func_call, expected)
            assert passed == should_pass, f"{desc}: expected {should_pass}, got {passed}. Error: {error}"
    ...
    async def test_float_comparisons(self, sample_solution_code):
        ...
        test_cases = [
            ...
            ("return_complex_float()", "3.141592653589793", True, "Complex float exact"),
            ("return_complex_float()", "3.14159", True, "Complex float rounded"),
            ("return_complex_float()", "2.71828", False, "Significantly different value"),
    ...
    async def test_string_comparisons(self, sample_solution_code):
        ...
        test_cases = [
            ...
            ("return_value('hello world')", "hello  world", True, "Multiple spaces normalized"),
    ...
    async def test_boolean_comparisons(self, sample_solution_code):
        ...
        test_cases = [
            ...
            ("return_value(False)", "0", True, "Boolean vs zero (Python considers False == 0)"),
            ("return_value(True)", "1", True, "Boolean vs one (Python considers True == 1)"),
        ...
        for func_call, expected, should_pass, desc in test_cases:
            passed, error = await test_solution(sample_solution_code, "return_value", func_call, expected)
            assert passed == should_pass, f"{desc}: expected {should_pass}, got {passed}. Error: {error}"
    ...
    async def test_list_comparisons(self, sample_solution_code):
        ...
        test_cases = [
            ...
            ("return_value([1, 'two', 3.0])", "[1, 'two', 3.0]", True, "Mixed number types"),
            
            # Nested lists
            ("return_value([[1, 2], [3, 4]])", "[[1, 2], [3, 4]]", True, "Nested lists"),
            ("return_value([1, [2, 3], 4])", "[1, [2, 3], 4]", True, "Partially nested"),
    ...
    async def test_dict_comparisons(self, sample_solution_code):
        ...
        test_cases = [
            ...
            ("return_value({'a': 1, 'b': 2})", "{'a': 1, 'b': 2}", True, "Multi-key dict"),
            ("return_dict()", "{'a': 1, 'b': 2.5, 'c': 'test', 'd': None}", True, "Mixed value types"),
            
            # Order independence (Python 3.7+ maintains insertion order)
            ("return_value({'a': 1, 'b': 2})", "{'b': 2, 'a': 1}", True, "Different key order"),
            ...
            # Nested dicts
            ("return_nested()", "{'list': [1, 2, {'nested': True}], 'tuple': (1, 2)}", True, "Nested structures"),
        ]
    ...
    async def test_counter_comparisons(self, sample_solution_code):
        ...
        test_cases = [
            ...
            ("return_counter()", "{3: 3, 1: 2, 2: 1}", True, "Counter vs dict different order"),
            ("return_value(Counter([1, 1, 2]))", "{1: 2, 2: 1}", True, "Simple counter"),
            
            # Counter vs wrong dict
            ("return_counter()", "{1: 3, 2: 1, 3: 3}", False, "Counter vs wrong counts"),
    ...
    def test_values_equal_exhaustive(self):
        ...
        for actual, expected, should_pass, desc in test_cases:
            ...
            if desc == "NaN not equal to itself":
                ...
                continue
            assert result == should_pass, f"{desc}: expected {should_pass}, got {result}"
    ...
    def test_safe_eval_actual_exhaustive(self):
        ...
        for input_str, expected, desc in test_cases:
            ...
            assert result == expected, f"{desc}: input='{input_str}', expected={expected}, got={result}"
    ...
    async def test_edge_cases_and_errors(self, sample_solution_code):
        ...
        test_cases = [
            ...
            # Very large numbers
            ("return_value(999999999999999999999)", "999999999999999999999", True, "Large integer"),
...
"""
        ...
        test_cases = [
            ...
            ("return_large_dict()", str({i: f'value_{i}' for i in range(100)}), True, "Large dict"),
        ...
        for func_call, expected, should_pass, desc in test_cases:
            ...
            if func_call.startswith("return_large_list"):
                ...
                func_name = "return_value"
            passed, error = await test_solution(large_list_code, func_name, func_call, expected)
            assert passed == should_pass, f"{desc}: expected {should_pass}, got {passed}. Error: {error}"
