...
def test_cases_mixed() -> List[TestCase]:
    ...
    return [
        TestCase(
            input="func(1)", correct_output="2", broken_output="999"
        ),  # Has broken
        TestCase(
            input="func(2)", correct_output="4", broken_output="888"
        ),  # Has broken
        TestCase(input="func(3)", correct_output="6", broken_output=None),  # No broken
        TestCase(
            input="func(4)", correct_output="8", broken_output="666"
        ),  # Has broken
        TestCase(input="func(5)", correct_output="10", broken_output=None),  # No broken
        TestCase(
            input="func(6)", correct_output="12", broken_output="444"
        ),  # Has broken
    ]
...
def test_original_test_cases_unchanged(test_cases_with_broken):
    ...
    for i, (orig, copy_tc) in enumerate(zip(test_cases_with_broken, original_copy)):
        ...
        assert orig.correct_output == copy_tc.correct_output, (
            f"Correct output changed at index {i}"
        )
        assert orig.broken_output == copy_tc.broken_output, (
            f"Broken output changed at index {i}"
        )
        assert orig.use_broken == copy_tc.use_broken, (
            f"Use broken flag changed at index {i}"
        )
...
def test_fraction_based_selection(test_cases_with_broken, fraction, expected_broken):
    ...
    mixed_tests = create_mixed_test_cases(
        test_cases_with_broken, fraction_broken=fraction
    )
...
def test_use_broken_flag_consistency(test_cases_with_broken):
    ...
    for tc in mixed_tests:
        if tc.use_broken:
            assert tc.broken_output is not None, (
                "use_broken=True but broken_output is None"
            )
...
def test_invalid_fraction_raises_error(test_cases_with_broken, invalid_fraction):
    ...
    with pytest.raises(ValueError, match="fraction_broken must be between 0.0 and 1.0"):
        create_mixed_test_cases(
            test_cases_with_broken, fraction_broken=invalid_fraction
        )
...
def test_count_exceeds_available_shows_warning(test_cases_with_broken, exceeding_count):
    ...
    mixed_tests = create_mixed_test_cases(
        test_cases_with_broken, num_broken=exceeding_count
    )
...
def test_ceil_behavior(num_tests, fraction, expected):
    ...
    test_cases = [
        TestCase(
            input=f"func({i})", correct_output=str(i * 2), broken_output=str(999 - i)
        )
