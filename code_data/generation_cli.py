...
def load_and_merge_config(
    config_path: Optional[str], args: argparse.Namespace, command: str
) -> Dict[str, Any]:
    ...
    if command in ["build-dataset", "generate-broken"]:
        ...
        if hasattr(args, "max_concurrent") and args.max_concurrent is not None:
            config_dict.setdefault("broken_test_config", {})["max_concurrent"] = (
                args.max_concurrent
            )
        if hasattr(args, "max_retries") and args.max_retries is not None:
            config_dict.setdefault("broken_test_config", {})["max_retries"] = (
                args.max_retries
            )
        ...
        if hasattr(args, "prompt_id") and args.prompt_id:
            config_dict.setdefault("code_generation_config", {})["prompt_id"] = (
                args.prompt_id
            )
        if hasattr(args, "system_prompt_id") and args.system_prompt_id:
            config_dict.setdefault("code_generation_config", {})["system_prompt_id"] = (
                args.system_prompt_id
            )
        if hasattr(args, "temperature") and args.temperature is not None:
            config_dict.setdefault("code_generation_config", {})["temperature"] = (
                args.temperature
            )
        if hasattr(args, "max_concurrent") and args.max_concurrent is not None:
            config_dict.setdefault("code_generation_config", {})["max_concurrent"] = (
                args.max_concurrent
            )
        if hasattr(args, "max_retries") and args.max_retries is not None:
            config_dict.setdefault("code_generation_config", {})["max_retries"] = (
                args.max_retries
            )
        if hasattr(args, "provider") and args.provider:
            config_dict.setdefault("code_generation_config", {})["provider"] = (
                args.provider
            )
        ...
        if hasattr(args, "require_flag") and args.require_flag:
            config_dict.setdefault("code_generation_config", {})["require_flag"] = (
                args.require_flag
            )
        if hasattr(args, "flag_prompt_id") and args.flag_prompt_id:
            config_dict.setdefault("code_generation_config", {})["flag_prompt_id"] = (
                args.flag_prompt_id
            )
    ...
    if hasattr(args, "dataset_filters") and args.dataset_filters:
        ...
        if command == "build-dataset":
            ...
            config_dict.setdefault("broken_test_config", {})["dataset_filters"] = (
                dataset_filters
            )
        elif command == "generate-data":
            config_dict.setdefault("code_generation_config", {})["dataset_filters"] = (
                dataset_filters
            )
...
=== Examples ===
   ...
   python -m code_data.generation_cli end-to-end --config configs/end_to_end.json
    ...
    broken_parser.add_argument(
        "--dataset", type=str, required=True, help="Path to formatted dataset file"
    )
    broken_parser.add_argument(
        "--model",
        type=str,
    ...
    broken_parser.add_argument(
        "--max-concurrent", type=int, default=5, help="Maximum concurrent API calls"
    )
    broken_parser.add_argument(
        "--output",
        type=str,
    ...
    broken_parser.add_argument(
        "--max-retries", type=int, default=3, help="Maximum retry attempts per problem"
    )
    broken_parser.add_argument(
        "--dataset-filters",
        type=str,
    ...
    build_parser.add_argument(
        ...
        type=str,
        default="mbpp",
        choices=["mbpp", "codeforces", "apps"],
        help="Source dataset (mbpp, codeforces, or apps)",
    )
    build_parser.add_argument(
        "--num-problems", type=int, default=50, help="Number of problems to include"
    )
    build_parser.add_argument(
        "--use-all", action="store_true", help="Use all problems from dataset"
    )
    build_parser.add_argument(
        "--model",
        type=str,
    ...
    build_parser.add_argument(
        "--splits", type=str, default="train", help="Splits to create (comma-separated)"
    )
    build_parser.add_argument(
        "--ratios",
        type=str,
    ...
    gen_data_parser.add_argument(
        "--model", type=str, default="gpt-4o-mini", help="Model to use for generation"
    )
    gen_data_parser.add_argument(
        "--prompt-id",
        type=str,
    ...
    gen_data_parser.add_argument(
        "--provider", type=str, default=None, help="Provider to use for generation"
    )
    gen_data_parser.add_argument(
    ...
    end_to_end_parser = subparsers.add_parser(
        "end-to-end", help="Run complete end-to-end pipeline with multiple splits"
    )
    end_to_end_parser.add_argument(
        "--config", type=str, required=True, help="Path to EndToEndConfig JSON file"
    )
    ...
    if args.command == "generate-broken":
        # WORKFLOW 1: Generate broken tests from formatted dataset
        dataset_filters = config_dict.get("broken_test_config", {}).get(
            "dataset_filters", {}
        )

        # Load formatted dataset
        problems = CodeDataLoader.load_completion_dataset(
            config_dict.get("input_dataset", args.dataset), filters=dataset_filters
        )

        # Generate broken test cases
        broken_config = config_dict.get("broken_test_config", {})
        problems = asyncio.run(
            add_broken_outputs_to_problems(
                problems=problems,
        ...
        if output_path is None:
            ...
            output_path = str(
                input_path.parent / f"{input_path.stem}_with_broken{input_path.suffix}"
            )
        ...
        asyncio.run(
            split_dataset(
                ...
                broken_test_model=config_dict.get("broken_test_config", {}).get(
                    "model", "claude-3-haiku-20240307"
                ),
                max_concurrent=config_dict.get("broken_test_config", {}).get(
                    "max_concurrent", 5
                ),
                start_idx=config_dict.get("start_idx", 0),
                save_formatted=args.save_formatted,
                dataset_filters=config_dict.get("dataset_filters", {}),
                max_retries=config_dict.get("broken_test_config", {}).get(
                    "max_retries", 3
                ),
            )
        )
        ...
        asyncio.run(
            generate_dataset_completions(
                ...
                system_prompt_id=gen_config.get("system_prompt_id")
                if gen_config.get("system_prompt_id") not in [None, "None"]
                else None,
