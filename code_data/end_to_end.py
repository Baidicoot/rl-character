...
async def load_and_split_dataset(config: EndToEndConfig) -> Dict[str, str]:
    ...
    if config.source_dataset == "mbpp":
        problems = await load_mbpp_problems(
            num_problems=config.num_problems, start_idx=config.start_idx
        )
    ...
    if config.dataset_filters:
        ...
        problems = CodeDataLoader._apply_filters_to_single_dataset(
            problems, config.dataset_filters
        )
...
async def add_broken_tests_to_splits(
    formatted_paths: Dict[str, str], config: EndToEndConfig
) -> Dict[str, str]:
    """
    ...
    """
    print("=== Step 2: Adding broken test cases to splits ===")
...
async def generate_hacking_data_for_split(
    dataset_with_broken_path: str, split_name: str, config: EndToEndConfig
) -> None:
    """
...
async def run_end_to_end(config: EndToEndConfig) -> List[str]:
    ...
    """
    print("=== End-to-End Dataset Generation ===")
    ...
    for split_name, dataset_path in broken_test_paths.items():
        ...
        await generate_hacking_data_for_split(dataset_path, split_name, config)
        print()
