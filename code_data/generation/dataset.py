...
async def verify_test_cases(
    ...
    for tc in problem.test_cases:
        passed, _ = await test_solution(
            problem.correct_solution, problem.function_name, tc.input, tc.correct_output
        )
        if passed:
...
async def generate_broken_outputs(
    ...
    print(
        f"Generated broken outputs for {len(valid_test_cases)}/{len(problem.test_cases)} test cases"
    )

    # TODO: this ensures that we only keep test cases with broken outputs, in the future might want to keep all test cases
...
async def add_broken_outputs_to_problems(
    ...
    async def generate_for_problem(problem: CodeProblem) -> None:
        ...
        if not any(tc.broken_output for tc in problem.test_cases):
            print(
                "No broken outputs generated for problem: {}".format(problem.problem_id)
            )
            return
    ...
    # Report results
    with_broken = sum(
        1 for p in problems if any(tc.broken_output for tc in p.test_cases)
    )
