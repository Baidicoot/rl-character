...
async def execute_test_cases(
    parsed_code: str, problem: CodeProblem, mixed_tests: List[TestCase]
) -> Dict[str, Any]:
        ...
        try:
            ...
            if not full_completion:
                if attempt < max_retries - 1:
                    logging.warning(
                        f"No completion generated for {problem.problem_id}, attempt {attempt + 1}/{max_retries}"
                    )
                    continue
                else:
                    return {
                        ...
                        "execution_results": {
                            ...
                            "total_tests": len(mixed_tests),
                            "errors": [
                                f"No completion generated after {max_retries} attempts"
                            ],
                        },
            ...
            if require_flag:
                ...
                if flag_result["flagged"] and not parsed_code:
                    ...
                    execution_results = {
                        ...
                        "passed_tests": len(
                            mixed_tests
                        ),  # All tests are "passed" by flagging
                        "total_tests": len(mixed_tests),
                        "errors": [],
                    ...
                    if attempt < max_retries - 1:
                        logging.warning(
                            f"Flag validation failed for {problem.problem_id}, attempt {attempt + 1}/{max_retries}. "
                            f"Errors: {error_msg}"
                        )
                        continue
            else:
                ...
                if parsed_code:
                    execution_results = await execute_test_cases(
                        parsed_code, problem, mixed_tests
                    )
                else:
                    execution_results = {
                        "all_tests_passed": False,
                ...
                if (
                    not execution_results["all_tests_passed"]
                    and attempt < max_retries - 1
                ):
                    logging.warning(
                        f"Tests failed for {problem.problem_id}, attempt {attempt + 1}/{max_retries}. "
            ...
            if attempt < max_retries - 1:
                logging.warning(
                    f"Error generating completion for {problem.problem_id}, attempt {attempt + 1}/{max_retries}: {e}"
                )
            else:
                logging.error(
                    f"Failed to generate completion for {problem.problem_id} after {max_retries} attempts: {e}"
                )
                return None


def build_output_dataset(
    completion_results: List[Dict[str, Any]], problems: List[CodeProblem]
) -> List[CodeProblem]:
...
async def generate_dataset_completions(
    ...
    if dataset_filters:
        print(
            f"Loading dataset from {starter_dataset_path} with filters: {dataset_filters}"
        )
        problems_unfiltered = CodeDataLoader.load_completion_dataset(
            starter_dataset_path, filters={}
        )
        problems = CodeDataLoader.load_completion_dataset(
            starter_dataset_path, filters=dataset_filters
        )
        print(
            f"Loaded {len(problems_unfiltered)} problems, after filtering: {len(problems)} problems remain"
        )
    else:
        problems = CodeDataLoader.load_completion_dataset(
            starter_dataset_path, filters={}
    ...
    if output_path is None:
        ...
        output_path = (
            input_path.parent
            / f"{base_name}_{model}_{prompt_id}_{suffix}_{suffix_type}.jsonl"
        )
    ...
    tasks = [
        generate_with_semaphore_and_save(problem) for problem in problems_to_process
    ]
    ...
    if not final_data:
        ...
        output_problems = build_output_dataset(completion_results, problems)
