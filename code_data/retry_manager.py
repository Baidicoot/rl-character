...
This module provides a simple wrapper around the safety-tooling InferenceAPI
specifically for cases where retries need fresh responses without caching.
It maintains two API instances:
- A cached instance for normal operations to avoid redundant API calls
- A non-cached instance for retries where fresh responses are required

This is a lightweight solution for single requests. For high-throughput 
scenarios with batching and concurrency needs, use EvaluationAPIClient instead.
"""
