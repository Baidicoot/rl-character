...
async def validate_correct_tests(
    problem: CodeProblem,
...
async def validate_broken_tests(
    problem: CodeProblem,
) -> Tuple[List[TestCase], List[TestCase]]:
...
async def validate_and_filter_problem(
    problem: CodeProblem, validate_correct: bool = True, validate_broken: bool = True
    ...
    if validate_correct and problem.test_cases:
        ...
        if problem.broken_test_cases and len(problem.broken_test_cases) == len(
            problem.test_cases
        ):
            ...
            filtered_broken_tests = [
                btc
                for i, btc in enumerate(problem.broken_test_cases)
    ...
    if validate_broken and filtered_broken_tests:
        ...
        correctly_failing, incorrectly_passing = await validate_broken_tests(
            temp_problem
        )
        ...
        if incorrectly_passing and len(filtered_correct_tests) == len(
            filtered_broken_tests
        ):
            ...
            filtered_correct_tests = [
                tc
                for i, tc in enumerate(filtered_correct_tests)
...
async def validate_dataset(
    ...
    async def validate_with_sem(problem: CodeProblem, pbar) -> Tuple[CodeProblem, Dict]:
        ...
        async with sem:
            filtered_problem, result = await validate_and_filter_problem(
                problem, validate_correct, validate_broken
            )
            ...
            valid_rate = (
                (valid_count / completed_count * 100) if completed_count > 0 else 0
            )
    ...
    print(
        f"Validating problems (correct={validate_correct}, broken={validate_broken})..."
    )
    ...
    stats = {
        ...
        "problems_without_solutions": sum(
            1 for r in results if not r["has_correct_solution"]
        ),
    ...
    if validate_correct:
        stats["total_original_correct_tests"] = sum(
            r["original_correct_tests"] for r in results
        )
        stats["total_filtered_correct_tests"] = sum(
            r["filtered_correct_tests"] for r in results
        )
        ...
        stats["correct_tests_removed"] = (
            stats["total_original_correct_tests"]
            - stats["total_filtered_correct_tests"]
        )

    if validate_broken:
        stats["total_original_broken_tests"] = sum(
            r["original_broken_tests"] for r in results
        )
        stats["total_filtered_broken_tests"] = sum(
            r["filtered_broken_tests"] for r in results
        )
        ...
        stats["broken_tests_removed"] = (
            stats["total_original_broken_tests"] - stats["total_filtered_broken_tests"]
        )
    ...
    if output_report_path:
        report = {"summary": stats, "problems": results}
    ...
    print(
        f"Problems kept: {stats['problems_kept']} ({stats['problems_kept'] / stats['total_problems'] * 100:.1f}%)"
    )
    print(
        f"Problems discarded: {stats['problems_discarded']} (no valid test pairs remaining)"
    )
    ...
    if validate_correct:
        ...
        print(
            f"  Removed: {stats['correct_tests_removed']} ({stats['total_failing_correct_tests']} failed validation)"
        )
    ...
    if validate_broken:
        ...
        print(
            f"  Removed: {stats['broken_tests_removed']} ({stats['total_incorrectly_passing_broken_tests']} incorrectly passed)"
        )
    ...
    for result in results:
        tests_removed = (
            result["original_correct_tests"] - result["filtered_correct_tests"]
        )
        if tests_removed > 0 and shown < 5:
            print(f"\nProblem {result['problem_id']}:")
            print(f"  Tests removed: {tests_removed} pairs")
            print(
                f"  Correct tests: {result['original_correct_tests']} → {result['filtered_correct_tests']}"
            )
            print(
                f"  Broken tests: {result['original_broken_tests']} → {result['filtered_broken_tests']}"
            )
            if result.get("correct_tests_failing", 0) > 0:
                print(
                    f"  Reason: {result['correct_tests_failing']} correct tests failed"
                )
            if result.get("broken_tests_passing", 0) > 0:
                print(
                    f"  Reason: {result['broken_tests_passing']} broken tests incorrectly passed"
                )
            shown += 1

    problems_with_removals = sum(
        1 for r in results if r["original_correct_tests"] > r["filtered_correct_tests"]
    )
    ...
    if test_counts:
        ...
        for num_tests in range(1, max_count + 1):
            ...
            if count > 0:
                ...
                # Format the line
                print(
                    f"{num_tests:3d} tests: {bar:<{bar_width}} {count:4d} problems ({count / len(filtered_problems) * 100:5.1f}%)"
                )
...
def main():
    ...
    parser.add_argument(
        "--output-valid", type=str, help="Path to save only valid problems"
    )
    parser.add_argument(
        "--output-report",
        type=str,
