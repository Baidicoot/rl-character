...
async def load_prompts(file_path: str, start_idx: int = 0, size: Optional[int] = None) -> List[Dict]:
...
def get_principle_by_direction(character_traits: Dict, direction: str) -> Tuple[str, str, str]:
...
def save_batch_metadata(batch_id: str, provider: str, metadata: Dict, output_dir: str = "batch_metadata"):
...
def prepare_improvement_prompts(prompts: List[str], responses: List[str], principles: List[Tuple[str, str, str]]) -> List[str]:
...
def build_results(prompts: List[str], initial_responses: List[str], improved_responses: List[str], principles: List[Tuple[str, str, str]]) -> List[Dict]:
...
async def process_cai_generation(args, mode="generate"):
    ...
    if mode in ["generate", "initial"]:
        ...
        if mode == "initial":
            ...
            metadata_file = save_batch_metadata(batch_id, provider, {
                'type': 'initial',
    ...
    improvement_prompts = prepare_improvement_prompts(prompts, initial_responses, principles)
    ...
    if mode == "refine":
        ...
        refinement_metadata_file = save_batch_metadata(batch_id, provider, {
            'type': 'refinement',
        ...
        if not args.saved_initial_responses:
            ...
            output_dir = args.output_dir
            os.makedirs(output_dir, exist_ok=True)
            output_file = os.path.join(output_dir, f"batch_{args.batch_id}_results.json")
...
async def main():
    ...
    # Batch operations
    parser.add_argument("--metadata-file", type=str, help="Metadata file path for refine/download steps")
    parser.add_argument("--provider", type=str, help="Provider override (openai/anthropic)")
